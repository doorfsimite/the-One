...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	...
	//---------------------ACTIVE OR DEACTIVE STATE------------------------------
		...
		//----------------------------DIOGO----------------------------------------------------
			...
			/** Interface ligada ao iniciar a interface**/
	...
	//-----------------------------------------------------------------------
	public boolean setEgoism(boolean mode) {
		if(this.egoist ^ mode ) {
			this.egoist = mode;
			return true;
		}
	...
	public void setHost(DTNHost host) {
		...
		if (!comBus.containsProperty(SCAN_INTERVAL_ID) &&
			...
			comBus.addProperty(core.SimScenario.SELFISH_MODE, this.egoist);
			comBus.subscribe(core.SimScenario.SELFISH_MODE, this);
	...
	public void setTransmitRange(double newRange) {
		this.oldTransmitRange = newRange;
	}
	
	//-- diz se a interface ta on ou off --------------------------------------------------
	
	public String getInterfaceStatus(){ return this.interfaceStatus == 0 ? "OFF" : "ON"; }
	public int getInterfaceState() {return this.intefaceState;}
	//-------------------------------------------------------------------------------------
	...
	public boolean isTx() {
		...
		//nao deve ser chamada antes de isTransferring()
		return false;
	}
	...
	public void moduleValueChanged(String key, Object newValue) {
		...
		else if (key.equals(core.SimScenario.SELFISH_MODE)) {
			setEgoism((boolean) newValue);
		}
		else {
