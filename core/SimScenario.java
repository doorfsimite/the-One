...
public class SimScenario implements Serializable {
	...
	protected SimScenario() {
		...
		//egoismo baseado em bateria é definido em EnergyModel
	...
	//----------------------------------------------------------------
	public void setEgoistHosts(List<DTNHost> hosts) {
		int egoistHosts = (genericEgoism*hosts.size())/100;
		while(egoistHosts > 0) {
			int index = random.nextInt(hosts.size());
			while(!hosts.get(index).setEgoism()) {
				index = random.nextInt(hosts.size());
			}
			System.out.println("host is egoist:"+index);
			egoistHosts -=1;
		}
	}
	
	public void showEgoists() {
		for (DTNHost host : this.hosts) {
			String mode;
			mode = (host.isEgoist() ? "EGOIST" :  "NORMAL");
			System.out.println(host.getAddress() + ": "+mode);
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			//System.out.println( mmProto.showHostMap());
