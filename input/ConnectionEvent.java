...
public class ConnectionEvent extends ExternalEvent {
	...
	public void processEvent(World world) {
		...
		//Verifica se a interface esta ligada antes de fazer a conexão, desse modo interfaces descarregadas não irao fazer desconexõe
		if(this.isUp && from.isRadioActive() && to.isRadioActive()) {
			from.forceConnection(to, interfaceId, this.isUp);
		}
		else {
			from.forceConnection(to, interfaceId, false);
		}
		//---------------------------------------------------------------------------------------------------------------------------
	}
