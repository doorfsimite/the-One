...
//----------------------------------------------------------------------------------------------------------------------------
		if (energy != null) {
			...
			if(this.getHost().isEgoist() && !this.getHost().equals(m.getTo())) {
				//System.out.println(this.getHost().getAddress() + " nao recebeu mensagem de "+m.getFrom().getAddress()+" | egoista");
				return MessageRouter.EGOIST;
			}
			if(this.energy.isLowBattery() && !this.getHost().equals(m.getTo())) {
				//System.out.println(this.getHost().getAddress() + " nao recebeu mensagem de "+m.getFrom().getAddress()+" | pouca bateria");
				return MessageRouter.EGOIST_ENERGY_LEVEL;
			}
...
//----------------------------------------------------------------------------------------------------------------------------		
	...
	public double getRechargeInterval() {
		if(this.energy!=null) {
			return this.energy.getRechargeInterval();
		}
		return 0.0;
	}
