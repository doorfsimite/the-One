...
public class DecisionEngineRouter extends ActiveRouter
{
	...
	public Message messageTransferred(String id, DTNHost from)
	{
		...
		if(aMessage.isMulticast()) {
			isFinalRecipient = aMessage.haveInToList(getHost());
			if (outgoing!=null ) {
		...
		if (isFirstDelivery)
		{
			deliveredMessagesPutting(id, aMessage);
		}

		List<MessageListener> mListenersCopy = getmListeners();
		for (MessageListener ml : getmListeners()) {
	...
	public void update()
	{
			...
			i.hasNext();)
		{
			...
			{
				i.remove();//So da remove se a mensagem chegou na comunidade do destino
			}
		}
	}
	/** E chamado antes do update terminar de remover a mensagem do outgointMessages*/
	public double haveDeliverableMessages(DTNHost to) {
		if(to == this.getHost()) {return 0.0;}
		double messagesSize = 0;
		for(Iterator<Tuple<Message, Connection>> i = outgoingMessages.iterator(); i.hasNext();){
			Tuple<Message, Connection> t = i.next();
			if(to == t.getValue().getOtherNode(this.getHost()) && this.hasMessage(t.getKey().getId())) {
				messagesSize += t.getKey().getSize();
			}
		}
		return messagesSize;
	}
